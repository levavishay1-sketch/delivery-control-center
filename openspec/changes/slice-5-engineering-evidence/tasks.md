...
- [x] 9.1 Write `e2e/slice5-engineering-evidence.spec.ts`: link a repository (stub GitHub API server per Slice 4's `startStubJiraServer` pattern — required adding a configurable `baseUrl` to `github.ts`, mirroring `jira.ts`, so the stub is reachable), link the catch-up-fetched merged PR to a work item via the Code & Changes tab, attempt `APPROVED → COMPLETED` and see it succeed; separately, attempt the same transition on a work item with no evidence and see it rejected, then approve a `CompletionException` via the Evidence tab and see it then succeed. (Evidence is linked via the catch-up fetch + manual UI link rather than a live webhook delivery — webhook-triggered recording itself has its own dedicated unit-test coverage in Task Group 4; this E2E's job is verifying the full user-facing flow end to end.) Passing standalone and in the full suite (8/9 — the only failure is the pre-existing, previously-flagged unrelated `slice3-budget-enforcement.spec.ts` flake).

## 10. Documentation & verification

- [x] 10.1 Update `docs/PRODUCT_SPEC.md`: revision header (six slices), §17 (Git/code/PR workflow) and §19 (Evidence and completion rules) rewritten from "unchanged"/unbuilt to implemented; §4's 360° Record item, §30 limitations, "Current capabilities"/"Missing capabilities", "Prioritized roadmap", and "Current Product Definition" all refreshed.
- [x] 10.2 Update `docs/ROADMAP.md`: gap-register items #37/#38 marked KEEP (✓ Slice 5), item #14 and #23 annotations updated, Slice 5 row → Done with archive link, Slice 5 section heading → "— **Done**" with archive detail, master-goal summary paragraph corrected (was self-contradictory — listed Slice-1-shipped layers under "Missing").
- [x] 10.3 Verification: `npx tsc --noEmit` clean, `npx eslint .` clean, `npm run build` succeeds, full unit suite 253/253 passing, full Playwright E2E suite 8/9 passing (the only failure is the pre-existing, previously-flagged unrelated `slice3-budget-enforcement.spec.ts` timeout — not touched by this slice). Live-checked via the new E2E test itself, which exercises the completion-policy rejection/approval path end to end through the running app (webhook signature verification has its own dedicated unit coverage from Task Group 4 — `recordCheckRunEvent`/`recordPullRequestEvent`/etc. — plus the pre-existing `route.ts` signature-verification behavior carried over unchanged from Slice 4).
