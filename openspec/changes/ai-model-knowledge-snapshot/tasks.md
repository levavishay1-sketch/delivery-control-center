...
- [x] 1.1 Add `ModelSnapshot` model to `prisma/schema.prisma` (`id`, `fetchedAt DateTime`, `status`
      enum `SUCCESS`/`FAILED`, `rawContent String`, `extractedModels Json`, `failureReason
      String?`), per design.md Decision 7.
- [x] 1.2 Add `FETCH_MODEL_SNAPSHOT` to the `JobType` enum.
- [x] 1.3 Run `npx prisma migrate dev --name ai_model_knowledge_snapshot`, regenerate the client,
...
- [x] 2.1 Add an optional `scheduledAt?: Date` parameter to `enqueueJob`
...
- [x] 2.2 Add a `nextSunday07UTC(from: Date): Date` helper (or inline equivalent) computing the
      next Sunday 07:00 UTC strictly after `from`.
- [x] 2.3 Add `ensureModelSnapshotJobScheduled()` to `src/domain/model-snapshot/commands.ts`: if no
...
- [x] 3.1 Add `src/lib/integrations/modelKnowledgeSource.ts` (or similar): `fetchModelSnapshotSource()`
...
- [x] 3.2 In the same module, add `extractModelFacts(rawHtml: string)`: strip HTML tags, scan for
...
- [x] 3.3 Unit tests for `extractModelFacts` against a handful of representative fixture HTML
...
- [x] 4.1 Add `recordModelSnapshotAttempt` (`src/domain/model-snapshot/commands.ts`): given
...
- [x] 4.2 Add `getLatestSuccessfulModelSnapshot` (`src/domain/model-snapshot/queries.ts`): returns
      the most recent `ModelSnapshot` with `status: SUCCESS`, or `null` if none exists.
- [x] 4.3 Add `recommendModel(ctx, workItemId)` (`src/domain/model-snapshot/queries.ts`): resolves
...
- [x] 5.1 Add `handleFetchModelSnapshotJob` to `worker.ts`: calls `fetchModelSnapshotSource` +
...
- [x] 5.2 Add `handleFetchModelSnapshotExhausted` to `worker.ts`: records a `FAILED`
...
- [x] 5.3 Register both handlers in `worker.ts`'s `handlers` map under `FETCH_MODEL_SNAPSHOT`.
- [x] 5.4 Call `ensureModelSnapshotJobScheduled()` once at `worker.ts` startup, before `pollLoop()`
      begins (design.md Decision 3).

## 6. API route

- [x] 6.1 Add `GET /api/work-items/[id]/recommendation/model` (or fold into the existing
...
- [x] 7.1 Add a second `AiRecommendationCard` call site on the Overview tab
      ...
      (Required generalizing `AiRecommendationCard` itself into a `kind: "executor" | "model"`
      component, since it was previously hardcoded to the AI_AGENT/HUMAN executor shape — not
      anticipated explicitly in design.md, but necessary to fulfill the proposal's own "reuse the
      shared card" commitment and the design-system spec's "duplicate status/action components are
      consolidated" requirement rather than forking a near-identical second component.)
- [x] 7.2 Render the snapshot's fetch date in the card as the freshness indicator, per the
      `ai-recommendation` spec's "shows how current that source is" scenario.

## 8. Tests

- [x] 8.1 Unit tests for `ensureModelSnapshotJobScheduled` (schedules when none pending, no-op when
      one is already `QUEUED`/`RUNNING`) and the `enqueueJob` `scheduledAt` parameter
      (`src/domain/job/commands.test.ts`).
- [x] 8.2 Unit tests for `recommendModel` (`src/domain/model-snapshot/queries.test.ts`): confirms
...
- [x] 8.3 Unit tests for `recordModelSnapshotAttempt` (success and failure paths).

## 9. E2E test scenario

- [x] 9.1 Add `e2e/ai-model-knowledge-snapshot.spec.ts`: seeds a `ModelSnapshot` directly (a real
      ...
      (Playwright's own bundler can't import `@/lib/db` directly — the generated Prisma client's
      ESM `import.meta` usage fails under its transform — so seeding runs via a standalone `tsx`
      fixture script, `e2e/fixtures/seedModelSnapshot.ts`, invoked as a subprocess; `tsx` already
      handles `@/` aliases and ESM fine, same as `worker.ts`. Also added a second scenario for the
      no-snapshot-yet fallback path, and fixed a real pre-existing test:
      `e2e/ai-recommendation-card.spec.ts` asserted the AI recommendation card fully disappears
      after assigning to AI — now a different card (the model recommendation, sharing the same
      aria-label) legitimately renders in its place, so the assertion was narrowed to confirm only
      that the executor-specific "Assign to AI" button is gone.)
