...
The system SHALL let a write-capable user link a GitHub repository to a
project through that project's `Connector`, recording it as a
`Repository`.

#### Scenario: Linking a repository
- **WHEN** a write-capable user links a GitHub repository to a project with a configured GitHub connector
- **THEN** a `Repository` record is created for that project, and the system begins accepting webhook events and fetching data for it
