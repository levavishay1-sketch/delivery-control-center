...
- [x] 4.1 `src/domain/connector/provenance.ts`: `recordSyncProvenance(workItemId, field, externalId)` and `recordManualProvenance(workItemId, field, actorUserId)` — upsert `FieldProvenance` by `[workItemId, field]`.
- [x] 4.2 `src/domain/work-item/commands.ts`'s manual-edit path (`updateWorkItem` or equivalent): after writing a field a sync can also write (`title`/`description`/`status`/`externalUrl`), calls `recordManualProvenance` for each touched field, in the same transaction as the write.
