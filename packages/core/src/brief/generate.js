...
import { summariseTimeline } from "../summarise.js";
/**
 ...
 * every new event.
 */
export async function regenerateBrief(clientId, workitemId) {
    const model = await withTenant(clientId, async (tx) => {
    ...
    await withTenant(clientId, (tx) => tx
        .insert(contextBrief)
        .values({
        workitemId,
        clientId,
