...
2. OPERATING MODEL: inspect representative and structurally important files only (entry points, manifests, the biggest modules, one example per pattern). Never attempt full comprehension; never list every class or file. Identify: components (with paths), entry_points (with how to run), boundaries, key_flows, integrations (system, direction, where implemented, compatibility constraints), generated_or_protected_areas (generated code, vendored code, infrastructure, sensitive), build_test commands (only ones you found in scripts/manifests/CI — set confidence honestly; never invent a command), constraints (repository-specific, non-obvious, with evidence), where_to_look (for common task types: "add an API endpoint", "change a data model", "fix a plugin", … → the paths), and existing_instructions_assessment.
   ...
   On that SAME read, also judge the file's FORM, not only its content, and set \`reshape\`: "none" (the file is the right kind of thing for its job), "supersede_with_skill" (its content belongs in an on-demand skill — typically a loose docs/*.md knowledge file written before skills were an option; set \`reshape_target\` to the skill that should own it), "consolidate" (it overlaps another artifact covering the same paths/topic; set \`reshape_target\` to the one to keep), or "redundant" (its job is already fully done elsewhere and nothing here is worth keeping). Content accuracy and form are independent: an accurate file can still be the wrong shape, and a stale one can still be the right shape. Explain in \`reshape_note_he\`, in Hebrew, one or two sentences. Default to "none" unless the case is clear — never propose reshaping something just because it is old.
...
5. EXISTING SETUP SUMMARY: write \`existing_setup_summary_he\`, in Hebrew, one short paragraph a non-technical reader understands: how much AI configuration this repository already carries, how much of it still holds up against the code, and what shape it is in overall. This is the holistic picture, not a list — the per-file detail already lives in existing_instructions_assessment.
...
- legacy_artifact: ONLY for a file that already exists and is none of the kinds above — typically a loose docs/*.md knowledge file predating on-demand skills. Propose one for every discovery entry whose \`reshape\` is not "none". action "remove" when another item in THIS plan takes its job over (set superseded_by to that item's path or skill name, and say so in title_he/justification), or "skip" to leave it alone with the reason why. This is the only kind that may touch a file a person wrote and the only one allowed action "remove" — never propose it for a file whose content is still doing a job nothing else covers, and never as tidying. A person opts into each removal explicitly at the gate; an unchecked one simply stays.

Lead \`rationale_he\` with the holistic picture of what this repository already carries before the per-item detail: how much AI configuration exists, how much still holds up, and what is being proposed about it overall (kept / updated / superseded). Discovery's existing_setup_summary_he is your starting point — do not just copy it, reconcile it with what you are actually proposing. Then the specifics.
