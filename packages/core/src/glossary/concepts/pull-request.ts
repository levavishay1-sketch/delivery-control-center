...
export const PULL_REQUEST_CONCEPTS: Concept[] = [
  ...
  { key: "merge", kind: "button", title: "מזג", aliases: ["מיזוג", "merge", "למזג"], screens: ["pull_request"], explain: "מחבר את הענף של הבקשה לענף היעד. זו פעולה של האתר המארח (GitHub / Azure DevOps) בשם שלכם.", press: "המיזוג מתבצע באתר המארח ואי אפשר לבטל אותו בלחיצה. הכפתור זמין רק כשיש סקירה מאושרת, אין קונפליקט, והבקשה שמעליה (אם יש) כבר מוזגה." },
  ...
  { key: "conflict", kind: "term", title: "קונפליקט", aliases: ["conflict", "התנגשות", "קונפליקטים"], screens: ["pull_request"], explain: "שני ענפים שינו את אותן שורות בצורה שונה, ו-git לא יודע איזו לקחת. מישהו צריך להחליט ידנית." },
  ...
  { key: "pr_blockers", kind: "section", title: "מה חוסם מיזוג", screens: ["pull_request"], explain: "כל מה שעוצר את המיזוג כרגע — סקירה חסרה, בדיקה שנכשלה, קונפליקט, או בקשה קודמת שעוד לא מוזגה. המיזוג נפתח רק כשאין שורה אדומה." },
  ...
  { key: "pr_conflict_files", kind: "term", title: "הקבצים בקונפליקט", aliases: ["קבצים בקונפליקט", "קבצים מתנגשים", "קבצים בהתנגשות", "הקבצים בהתנגשות"], screens: ["pull_request"], explain: "הקבצים שהענף והיעד שינו בהם את אותן שורות. לחיצה על השורה 'קונפליקט מול היעד' פותחת אותם בלשונית קבצים, תחת 'בקונפליקט'. 'חשודים' = ששני הצדדים שינו, ולא בטוח שהקונפליקט בכולם." },
  { key: "pr_conflict_result", kind: "section", title: "התוצאה", aliases: ["הקובץ אחרי ההכרעה"], screens: ["pull_request"], explain: "הקובץ כפי שיישמר אחרי מה שבחרתם בכל מקום. הוא מתעדכן עם כל בחירה, וזה בדיוק הטקסט שיידחף — לא גרסה מקורבת שלו." },
  { key: "pr_conflict_manual", kind: "section", title: "עריכת הקובץ שלכם", aliases: ["ערוך ידנית", "עריכה ידנית"], screens: ["pull_request"], explain: "הקובץ מהענף שלכם, פתוח לעריכה חופשית. מה שתכתבו כאן הוא מה שיישמר, ולכן צריך ללחוץ 'אשר עריכה' כדי לומר שסיימתם. עד האישור הקובץ נחשב לא מוכרע." },
  { key: "pr_conflict_check", kind: "section", title: "בדיקה שכלום לא נשבר", aliases: ["בדוק את התוצאה", "בדיקות אחרי הכרעה"], screens: ["pull_request"], explain: "הבדיקות שהמאגר עצמו מגדיר (typecheck, lint, test) רצות על תוצאת ההכרעה בעותק מבודד, לפני שמשהו נדחף. מאגר בלי בדיקות כאלה יאמר זאת במקום להציג סימן ירוק." },
  { key: "pr_conflict_save", kind: "section", title: "שמירת ההכרעה", aliases: ["שמור את הפתרון", "פתרון קונפליקט"], screens: ["pull_request"], explain: "מה שבחרתם נשמר כקומיט מיזוג אחד על הענף של הבקשה, בשמכם, ונדחף לגיט־האוסט. זו כתיבה למערכת חיצונית שאי אפשר לבטל מכאן; המיזוג לענף היעד עדיין נעשה בנפרד." },
  { key: "shared_files", kind: "field", title: "קבצים משותפים", screens: ["pull_request"], explain: "כמה מהשינויים שנוספו ליעד נוגעים באותם קבצים שהבקשה משנה. זה מה שקובע אם צפוי קונפליקט." },
  ...
  { key: "prs_attention", kind: "field", title: "דורש תשומת לב", screens: ["pull_request"], explain: "בקשות שמשהו בהן תקוע: בדיקה שנכשלה, קונפליקט, או המתנה ארוכה מדי לסקירה." },
  { key: "prs_ready", kind: "field", title: "מוכן למיזוג", screens: ["pull_request"], explain: "בקשות שעברו סקירה, הבדיקות שלהן ירוקות ואין בהן קונפליקט. נשאר רק להחליט למזג." },
