...
const FACT_TEMPLATES: { re: RegExp; keys: string[]; say: (v: unknown, key: string) => string }[] = [
  ...
  { re: /(אילו|איזה|באילו|באיזה|איפה|מה).{0,20}(קבצים|קובץ|מקומות).{0,25}(מתנגש|התנגשות|התנגשו)|איפה.{0,12}התנגשות|(קבצים|קובץ).{0,12}(מתנגש|בהתנגשות)/,
    keys: ["הקבצים בהתנגשות", "קבצים חשודים בהתנגשות"],
    say: (v, key) => Array.isArray(v) && v.length
      ? `${key.includes("חשודים") ? "אלה הקבצים ששני הצדדים שינו — ההתנגשות באחד מהם או יותר, ואין ודאות באיזה:" : `${v.length === 1 ? "קובץ אחד מתנגש" : `${v.length} קבצים מתנגשים`}:`}\n${v.map((x) => `- ${String(x)}`).join("\n")}`
      : String(v) },
