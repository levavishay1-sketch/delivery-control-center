...
const FACT_TEMPLATES: { re: RegExp; keys: string[]; say: (v: unknown, key: string) => string }[] = [
  ...
  // "קונפליקט" is the word the screens use; "התנגשות" is still understood, since people say both.
  { re: /(אילו|איזה|באילו|באיזה|איפה|מה).{0,20}(קבצים|קובץ|מקומות).{0,25}(מתנגש|התנגשות|התנגשו|קונפליקט)|איפה.{0,12}(התנגשות|קונפליקט)|(קבצים|קובץ).{0,12}(מתנגש|בהתנגשות|בקונפליקט)/,
    keys: ["הקבצים בקונפליקט", "קבצים חשודים בקונפליקט"],
    say: (v, key) => Array.isArray(v) && v.length
      ? `${key.includes("חשודים") ? "אלה הקבצים ששני הצדדים שינו — הקונפליקט באחד מהם או יותר, ואין ודאות באיזה:" : `${v.length === 1 ? "קובץ אחד בקונפליקט" : `${v.length} קבצים בקונפליקט`}:`}\n${v.map((x) => `- ${String(x)}`).join("\n")}`
      : String(v) },
