-- 0048 — two things the first real reading of a spec got wrong, fixed in
-- the prompt: a decision is a piece a task can be linked to (it was left
-- with no anchor, so every decision read as "nothing implements this"),
-- and a field's own requirements — audit history, a maximum length,
-- mandatory under a condition — are pieces of their own, instead of being
-- folded into the field, where a requirement nobody implemented is invisible.
-- (0046 carries the same text for a database created from scratch.)
UPDATE "prompt_template" SET "body" = $p$Index this requirement's specification so each piece of it can be pointed at, and say which task implements which piece. Read only — do not rewrite the spec.
...
Answer with the JSON object and nothing else.$p$, "body_he" = $p$בנה אינדקס לאפיון של הדרישה הזו, כך שאפשר יהיה להצביע על כל חלק בו, ואמור איזו משימה מממשת איזה חלק. קריאה בלבד — אל תשכתב את האפיון.
